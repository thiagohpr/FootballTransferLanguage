...
class Tokenizer():
    ...
    def selectNext(self):
        ...
        reservedWords = {'Here_we_Go':'pinit',
                        'End_of_Record':'pend',
                        'leak':'println',
                        'consider_rumour':'if',
                        'or_else_do':'else',
                        'begin_conversation':'while',
                        'value':'Int',
                        'player':'String',
                        'complete_transference':'return',
                        'begin_transference':'function',
                        'capture':'readline',
                        'is':'ASSIGN'
                        }
        ...
        if c in operationsTokens:
            ...
            if word in reservedWords.keys():
                if reservedWords[word] == 'Int' or reservedWords[word] == 'String':
                    self.next = Token(reservedWords[word], 'TYPE')
                elif word == 'is':
                    self.next = Token('=','ASSIGN')
                else:
                    self.next = Token(reservedWords[word],reservedWords[word].upper())
            else:
...
class Parser():
    ...
    def parseBlock():
        ...
        if token_atual.type == 'PINIT':
            Parser.tokenizer.selectNext()
            while token_atual.type !='PEND':
                statements.append(Parser.parseStatement())
                token_atual = Parser.tokenizer.next
                if token_atual.type == 'EOF':
                    raise ValueError('Não indicou final do programa.')
        else:
            raise ValueError('Não indicou início do programa.')
        ...
        if token_atual.type == 'LINE':
            ...
            if token_atual.type == 'IDEN':
                ...
                if Parser.tokenizer.next.type == 'IDEN':
                    ...
                    raise ValueError('Erro de definição de variável')
                ...
                if token_atual.value == '(':
                    ...
                    token_atual = Parser.tokenizer.next
                    if token_atual.value != ')':
                        raise ValueError('Não fechou parênteses do While')
                else:
                    raise ValueError('Não abriu parênteses para o While')
                ...
                if token_atual.value == '{':
                    ...
                    raise ValueError('Não abriu chaves no While!') 
                ...
                if token_atual.value == '(':
                    ...
                    token_atual = Parser.tokenizer.next
                    if token_atual.value != ')':
                        raise ValueError('Não fechou parênteses do If')
                else:
                    raise ValueError('Não abriu parênteses para o If')
                        ...
                        if token_atual.type == 'LINE':
                            ...
                            if token_atual.type=='ELSE':
                                ...
                                if token_atual.value == '{':
                                    ...
                                    raise ValueError('Não Abriu chaves no Else')
                            else:
                                #if com 2 filhos
                                # print('If 2 filhos')
                            ...
                            ValueError('Não pulou linha ao fechar chaves')
                    else:
                        raise ValueError('Não pulou linha ao abrir chaves')
                else:
                    raise ValueError('Não abriu chaves para o If')
                        ...
                        if token_atual.value == '(':
                            ...
                            if token_atual.value == ')':
                                ...
                                if token_atual.value == '{':
                                    ...
                                    raise ValueError('Não abriu chaves na declaração de função!')
                            else:
                                raise ValueError("Erro de sintaxe") 
                        else:
                            raise ValueError('Não abriu parênteses ao declarar função')
                    else:
                        raise ValueError('Erro de Parser: Declarar tipo da função!')
                else:
